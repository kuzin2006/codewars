...
def sum_pairs(ints, s):
    indices = []
    chosen_pair = []
    for idx in set([list(i) for i in permutations(range(len(ints)), 2)]):
        if ints[idx[0]] + ints[idx[1]] == s:
                # get pairs indices
                if chosen_pair:
                    if idx[1] - idx[0] < chosen_pair[1] - chosen_pair[0]:
                        chosen_pair = idx
                else:
                    chosen_pair = idx
    ...
    if not chosen_pair:
        ...
        return [ints[chosen_pair[0]], ints[chosen_pair[1]]]
